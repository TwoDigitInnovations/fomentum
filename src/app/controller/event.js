...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      if (req.body && req.body.image) {
        ...
        // await s3.upload({ file: req.body.image, filename: 'eventImage_' + event._id });
        event.image = img;
      } else {
        event.image = "";
      }
  ...
  getevents: async (req, res) => {
    try {
      if (req.body.category) {
        ...
        let eventI = await EventInterested.find({
          event: { $in: eventids },
        }).lean();
    ...
    try {
      ...
      if (req.body.image) {
        ...
        if (img) {
          ...
          newimg = req.body.image;
          // await s3.upload({ file: req.body.image, filename: 'eventImage_' + req.body.eventId })
        }
      } else {
        newimg = "";
      }
  ...
  addInterest: async (req, res) => {
    try {
      let eventInterested = await EventInterested.findOne({
        event: req.body.event,
      });

      if (!eventInterested) {
        eventInterested = await EventInterested.create({
          event: req.body.event,
          users: [req.user.id],
        });
  ...
  addGoing: async (req, res) => {
    try {
      ...
      if (!eventGoing) {
        eventGoing = await EventGoing.create({
          event: req.body.event,
          users: [req.user.id],
        });
  ...
  listGoing: async (req, res) => {
    try {
      let eventGoing = await EventGoing.findOne({
  ...
  listMyEvents: async (req, res) => {
    try {
      let eventInterested = await EventInterested.find(
        { users: { $elemMatch: { $eq: req.body.user_id || req.user.id } } },
        { users: 0 }
      ).populate({
        path: "event",
        select: "title",
  ...
  listGoMyEvents: async (req, res) => {
    try {
      let eventgoing = await EventGoing.find(
        { users: { $elemMatch: { $eq: req.body.user_id || req.user.id } } },
        { users: 0 }
      ).populate({
        path: "event",
